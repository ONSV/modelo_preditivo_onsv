...
df_total %>% 
  ...
  labs(x = NULL, y = NULL) +
  scale_x_continuous(limits = c(2007, 2023), breaks = seq(2006, 2023, 1)) +
...
novos_condtrs |>
  ggplot(aes(x = ano, y = novos_condutores)) +
    ...
    scale_x_continuous(breaks = seq(1996, 2023, 1), limits = c(2012,2023)) +
    scale_y_continuous(labels = comma_format(big.mark = ".")) +
    labs(x = NULL, y = NULL)
```
...
```{r}
#| fig.cap: Previsões com intervalo de confiança até 2024
#| echo: false
#| label: fig-pred
#| fig.width: 7
#| fig.height: 5
...
taxas_pred |> 
  ...
  scale_x_continuous(limits = c(2011, 2024), breaks = seq(1996, 2024, 1)) +
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1)) +
  labs(x = NULL, y = NULL) +
  scale_color_manual(
    labels = c(
...
É notável um aumento expressivo de vítimas previstas em comparação à previsão de 2022, assim como uma dilatação no intervalo de confiança para dados exteriores ao conjunto de ajuste. Este fato implica que o alto desempenho do mesmo modelo em 2022 possa ter sido parcial ao acaso, podendo este ocasionar em um valor menos preciso para o caso de 2023.
