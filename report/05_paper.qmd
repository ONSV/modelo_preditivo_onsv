...
A obtenção dos dados de mortalidade foram efetuados através do pacote `microdatasus` [@microdatasus], elaborado para a linguagem R. Coletou-se dados oriundos do período entre 1996 e 2023. Esta escolha de intervalo temporal visa englobar a mortalidade estimada sob o protocolo da CID-10, pelo qual o banco de dados foi adequado ao estudo extraindo apenas as observações de mortes relacionadas a sinistros, ou seja, aquelas que se encaixam entre os códigos "V01" à "V89".
...
Em razão da ausência de dados prévios à 2011 de certas fontes, o intervalo de estudo foi reduzido para 2011 a 2024.
...
df_total %>% 
  ...
  labs(x = NULL, y = NULL) +
  scale_x_continuous(limits = c(2007, 2024), breaks = seq(2006, 2024, 1)) +
...
novos_condtrs <- df_total |>
  mutate(novos_condutores = condutores - dplyr::lag(condutores)) |> 
...
novos_condtrs |>
  ggplot(aes(x = ano, y = novos_condutores)) +
    ...
    scale_x_continuous(breaks = seq(1996, 2024, 1), limits = c(2012,2024)) +
    scale_y_continuous(labels = comma_format(big.mark = ".")) +
    labs(x = NULL, y = NULL)
```
...
df_pred <- lm_wflow |> 
  predict(df_preprocess) |> 
  cbind(df_preprocess)

df_pred <- lm_wflow |> 
  predict(df_preprocess, type = "conf_int") |> 
  cbind(df_pred) |> 
...
mortes_previsao |> 
  ...
  geom_line(linewidth = 1, alpha = 0.25) +
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1)) +
  scale_x_continuous(breaks = seq(1996, 2024, 1)) +
  scale_y_continuous(labels = comma_format(big.mark = ".")) +
  labs(x = NULL, y = NULL) +
  scale_discrete_onsv()
```

O modelo foi utilizado para prever a ocorrência de mortes relacionadas à sinistros de trânsito em 2023, recebendo como *input* os respectivos dados das bases anteriormente consultadas. A predição aponta a tendência anual das mortes, com limites de erro considerados em um intervalo de confiança, conforme apresentado na @fig-pred. O valor previsto para 2023, com base no modelo efetuado, resultou em 34.725 mortes, com um intervalo de confiança entre 33.070 e 36.434 mortes.

```{r}
#| fig.cap: Previsões com intervalo de confiança até 2023
#| echo: false
#| label: fig-pred
#| fig.width: 7
#| fig.height: 5
...
Com o valor previsto para 2023, foi possível realizar cálculo das taxas de mortos por 100 mil habitantes e por 10 mil veículos, com o propósito de observar suas tendências. A @fig-hab explicita a crescente taxa de mortes por 100 mil habitantes, apresentando uma estimativa de 17,11 em 2023.
...
taxas_pred |> 
  ...
  scale_x_continuous(limits = c(2011, 2023), breaks = seq(1996, 2023, 1)) +
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1)) +
  labs(x = NULL, y = NULL) +
  scale_color_manual(
    labels = c(
  ...
  geom_label(
    data = taxas_pred |> filter(ano == 2023),
    aes(
      x = ano,
...
A taxa de mortes por 10 mil veículos resultou em uma previsão de 2,96 em 2023, como mostrada na @fig-veic. O valor representa uma queda em comparação aos anos anteriores, o que pode ser uma consequência do aumento da frota de veículos em relação às mortes no trânsito.
...
taxas_pred |>
  ...
  geom_point(shape = 21, size = 1.5, stroke = 1.5, aes(color = name)) +
  geom_line(linewidth = 1, alpha = 0.25, aes(color = name)) +
  scale_x_continuous(limits = c(2011, 2024), breaks = seq(1996, 2023, 1)) +
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1)) +
  labs(x = NULL, y = NULL) +
  scale_color_manual(
    labels = c(
  ...
  geom_label(
    data = taxas_pred |> filter(ano == 2024),
    aes(
      x = ano,
...
O valor obtido da previsão implica em uma leve queda de 7,4% no número de mortes em relação ao ano anterior (2022), que registrou 33.894 casos. Conforme a análise dos dados, por três anos consecutivos, a partir de 2020, houve um incremento no número de fatalidades no trânsito. O intervalo de confiança mostra como este pode ser o caso de 2024 novamente, não obstante da diminuição prevista para as mortes. Ademais, é notável que, apesar da intensidade das correlações dos preditores selecionados para o estudo, a resolução temporal anual permanece como um gargalo para a análise dos sinistros de trânsito, já que a disponibilidade de dados é limitada.

Retornando ao resultado obtido anteriormente de 34.752 óbitos previstos para 2023, é possível comparar o valor previsto ao valor consolidado real disponibilizado pelo DataSUS em 2024. Foram encontrados 34.881 óbitos, exibindo uma diferença de 156 vítimas esperadas por este modelo.

### Previsões para 2024

A divulgação dos dados consolidados de óbitos em trânsito para 2023 pelo DataSUS e a atualização das bases de dados do Senatran viabilizam a repaginação do modelo linear anual, passível de uma reestruturação para acomodar os novos dados com o objetivo de prever o total de vítimas no ano de 2024. Para o treinamento deste modelo, a mesma metodologia de treinamento é utilizada nas novas observações, sendo a previsão emitida na @fig-pred2024:
...
dados_modelo_2024 <- list(
  ...
  arrange(ano)|>
  mutate(
    frota = ifelse(ano == 2024, 123974520, frota)
  )
```
...
#| echo: false
#| label: fig-pred2024
#| fig.cap: Previsão de mortes anuais para 2024
#| fig.width: 7
#| fig.height: 5
...
pred_anual_2024 |> 
  ...
  ggplot(aes(ano, Mortes, color = Tipo)) +
    ...
    annotate("label",
             x = 2023,
             y = 40000,
             label = paste0("Total de 2024:\n", 
                            round(last(pred_anual_2024$.pred)),
                            " vítimas"))
```

É notável um aumento expressivo de vítimas previstas em comparação à previsão de 2023, assim como uma dilatação no intervalo de confiança para dados exteriores ao conjunto de ajuste. Este fato implica que o alto desempenho do mesmo modelo em 2023 possa ter sido parcial ao acaso, podendo este ocasionar em um valor menos preciso para o caso de 2024.
...
Em conformidade com a renovação dos dados das bases consultadas para o ano de 2023, é viável a atualização do modelo mensal para a emissão de uma nova previsão. Sendo assim, a escolha do modelo de regressão linear mensal para esta situação é decorrente da capacidade deste algoritmo em fornecer intervalos de confiança. As predições mensais de 2023 estão destacadas na @fig-predmes2023, assim como o total de vítimas de trânsito neste período:
...
```{r}
#| echo: false
#| label: fig-predmes2023
#| fig.cap: Previsão de mortes mensais para 2023
#| fig.width: 7
#| fig.height: 5

pred_soma_2023 <- pred_mensal_2023 |> 
  filter(year(data) == 2023) |> 
  select(.pred) |> 
  sum() |> 
  round()

ggplot(pred_mensal_2023, aes(data, mortes)) +
    geom_ribbon(
      aes(ymin = .pred_lower, ymax = .pred_upper),
      fill = "grey70",
      alpha = 0.7
    ) +
    geom_line(aes(y = .pred, color  = "Óbitos previstos"), size = 1) +
    geom_line(aes(color  = "Óbitos reais"), size = 1) +
    scale_x_date(limits = as.Date(c("2011-01-01", "2023-12-01")),
                 date_breaks = "2 years",
                 labels = label_date(format = "%Y")) +
    scale_color_manual(values = c(
      "Óbitos reais" = onsv_palette$blue,
      "Óbitos previstos" = onsv_palette$yellow)
    ) + 
    scale_y_continuous(labels = comma_format(decimal.mark = ",",big.mark = ".")) +
    xlab("Anos") +
    ylab("Mortes") +
    geom_label(x = as.Date("2022-01-01"), y = 3500, 
             label = paste0("Total de 2023:\n", pred_soma_2023, " vítimas"))
```

Como de praxe, os valores para as métricas de avaliação do modelo ajustado são evidenciados na @tbl-erlinreg2023, demonstrando a capacidade de aprendizado e predição do mesmo.

```{r}
#| echo: false
#| label: tbl-erlinreg2023
#| tbl-cap: Métricas de erro para regressão linear mensal de 2023

df_metricas_2023 <-
  bind_cols(
    test_2023,
    predict(lm_wflow_mensal_2023, test_2023)
  )

ml_metrics(df_metricas_2023, truth = mortes, estimate = .pred) |> 
  select(-.estimator) |> 
  mutate(.metric = toupper(.metric)) |> 
  rename(Métrica = .metric, Valor = .estimate) |>
  knitr::kable(table.attr = "quarto-disable-processing=true") |> 
